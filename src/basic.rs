...
impl Context {
    /// Grids and game boards. The subroutine call board (n1 , n2 , n3
    /// , n4 , piece , wrap , directed ) con- structs a graph based on
    /// the moves of generalized chesspieces on a generalized
    ...
    /// scanning the sequence (n1 , n2 , n3 , n4 , 0) = (n1, n2, n3,
    /// n4, 0) from left to right until coming to the first
    /// nonpositive parameter nk+1. If k = 0 (i.e., if n1 ≤ 0), the
    /// default size 8 × 8 will be used; this is an ordinary
    /// chessboard with 8 rows and 8 columns. Otherwise if nk+1 = 0,
    /// the board will have k dimensions n1, . . . , nk. Otherwise we
    /// must have nk+1 < 0; in this case, the board will have d =
    /// |nk+1| dimensions, chosen as the first d elements of the
    /// infinite periodic sequence (n1,...,nk,n1,...,nk,n1,...). For
    /// example, the specification (n1,n2,n3,n4) = (2,3,5,−7) is about
    /// as tricky as you can get. It produces a seven-dimensional
    /// board with dimensions (n1, . . . , n7) = (2, 3, 5, 2, 3, 5,
    /// 2), hence a graph with 2 · 3 · 5 · 2 · 3 · 5 · 2 = 1800
    /// vertices.
    ///
    ...
    /// coordinates k1, k2, . . . will wrap around when wrap = 2k1−1 +
    /// 2k2−1 + · · · . Setting wrap = −1 causes all coordinates to be
    /// computed modulo their size.
    ///
    /// The graph constructed by board will be undirected unless
    /// directed ̸= 0. Directed board graphs will be acyclic when wrap
    /// = 0, but they may have cycles when wrap ̸= 0. Precise rules
    /// defining the directed arcs are given below.
    ///
    ...
    /// graph on n vertices, you can say board(n,0,0,0,2,0,0). To get
    /// a circuit (undirected) or a cycle (directed) of length n, you
    /// can say board (n, 0, 0, 0, 1, 1, 0) and board (n, 0, 0, 0, 1,
    /// 1, 1), respectively.
    fn board(&mut self, mut n1: Long, mut n2: Long, mut n3: Long, mut n4: Long,
        ...
        // all-purpose indices
        let mut i: Long; let mut j: Long; let mut k: Long;
        let mut d: Long; // the number of dimensions
        let mut v: Vertex; // the current vertex of interest
        ...
        'done: loop { // (not really, just a trick to emulate goto)
            if piece == 0 {
                piece = 1;
            }
            ...
            if d.to_uint().unwrap() > MAX_D {
                panic!("bad_specs d: {}", d);
            }
            for (j, k) in (1..).zip(k..d+1) {
                self.nn[k as usize] = self.nn[j];
            }
        }
        ...
        let mut nnn : f32; // approximate size
        n = 1; nnn = 1.0;
        for j in 1..d+1 {
            nnn *= self.nn[j.to_uint().unwrap()] as f32;
            if nnn > MAX_NNN { panic!("very_bad_specs nnn: {}", nnn);}
            n *= self.nn[j.to_uint().unwrap()]; // this multiplication cannot cause integer overflow
        }
        unimplemented!();
